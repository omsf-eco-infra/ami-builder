...
@mock_aws
def test_list_managed_images_shows_managed_only(runner):
    ...
    result = runner.invoke(ami_cli.cli, ["list"])
...
@mock_aws
def test_list_managed_images_handles_empty(runner):
    result = runner.invoke(ami_cli.cli, ["list"])
    assert result.exit_code == 0
...
@mock_aws
def test_list_expired_filters_by_past_delete_after(runner, monkeypatch):
    ...
    result = runner.invoke(ami_cli.cli, ["list-expired"])
    assert result.exit_code == 0
...
@mock_aws
def test_list_expired_handles_no_expired(runner, monkeypatch):
    monkeypatch.setattr(ami_cli, "date", fixed_date())
    ec2 = boto3.client("ec2", region_name="us-east-1")
...
@mock_aws
def test_delete_images_removes_snapshots_by_default(runner):
    ec2 = boto3.client("ec2", region_name="us-east-1")
    ami_to_delete, snapshot_ids = create_managed_ami(ec2, "todelete")
    ami_to_keep, _ = create_managed_ami(ec2, "keep")

    result = runner.invoke(ami_cli.cli, ["delete", ami_to_delete])
    assert result.exit_code == 0
...
@mock_aws
def test_delete_images_can_skip_snapshot_removal(runner):
    ec2 = boto3.client("ec2", region_name="us-east-1")
    ami_to_delete, snapshot_ids = create_managed_ami(ec2, "todelete")

    result = runner.invoke(ami_cli.cli, ["delete", ami_to_delete, "--no-delete-snapshots"])
    assert result.exit_code == 0
...
def test_delete_images_handles_empty_response(runner, monkeypatch):
    class DummyClient:
        def describe_images(self, ImageIds):
            return {"Images": []}

    monkeypatch.setattr(ami_cli, "get_ec2_client", lambda: DummyClient())

    result = runner.invoke(ami_cli.cli, ["delete", "ami-12345678"])
    assert result.exit_code == 0
    assert "No matching AMIs found." in result.output


@mock_aws
def test_auto_delete_removes_expired_amis_and_snapshots(runner, monkeypatch):
    monkeypatch.setattr(ami_cli, "date", fixed_date())
    ec2 = boto3.client("ec2", region_name="us-east-1")
    ...
    result = runner.invoke(ami_cli.cli, ["auto-delete", "--force"])
    assert result.exit_code == 0
    remaining_ids = {img["ImageId"] for img in ec2.describe_images(Owners=["self"])["Images"]}
...
@mock_aws
def test_auto_delete_handles_no_targets(runner, monkeypatch):
    monkeypatch.setattr(ami_cli, "date", fixed_date())
    ec2 = boto3.client("ec2", region_name="us-east-1")
...
@mock_aws
def test_auto_delete_prompts_without_force(runner, monkeypatch):
    monkeypatch.setattr(ami_cli, "date", fixed_date())
    ec2 = boto3.client("ec2", region_name="us-east-1")
    ...
    result = runner.invoke(ami_cli.cli, ["auto-delete"], input="y\n")
    assert result.exit_code == 0
